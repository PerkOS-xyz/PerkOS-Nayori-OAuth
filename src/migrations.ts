...
const MIGRATION_LOCK_NAME = "perkos-nayori-oauth-migrations";
...
export async function loadMigrationFiles(directory: string): Promise<readonly MigrationFile[]> {
  const filenames = (await readdir(directory)).filter((name) => MIGRATION_FILE.test(name)).sort();
...
export async function runMigrations(pool: Pool, directory: string): Promise<void> {
  ...
  if (migrations.length === 0) throw new Error(`No migration files found in ${basename(directory)}.`);
  ...
  try {
    ...
    try { await client.query("SELECT pg_advisory_unlock(hashtext($1))", [MIGRATION_LOCK_NAME]); }
